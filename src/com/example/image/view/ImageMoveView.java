...
public class ImageMoveView
{
	...
	public void getHit(float dx, float dy)
	{
		...
		if (!((dx > right || dx < left) || (dy < top || dy > bottom))) // 点在涂鸦图片�?
		{
			// 向哪个方向移�?
			mCanMove = true;
	...
	private void checkBound(float dx, float dy)
	{
		...
		if (dx - mDeltaLeft < 0 || dy - mDeltaTop < 0 || (left + dx + mBitmap.getWidth() - mDeltaLeft) > right || dy + top + mBitmap.getHeight() - mDeltaTop > bottom) // 移动点超出组件范�?
		{
			mCanMove = false;
		}
